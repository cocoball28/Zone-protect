...
public class EnderManGriefListener {
    ...
    public void onEnderManTakeBlocks(ChangeBlockEvent.All event, @First Enderman enderman) {
        ...
        inZone
                .entrySet()
                .stream()
                .filter(entry -> entry.getKey().operation() == Operations.BREAK.get() || entry.getValue().getFlag(FlagTypes.ENDER_MAN_GRIEF).isPresent())
