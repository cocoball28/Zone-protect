...
public class DefaultTitleFadeInNode implements ZoneNode.WithDefault<TimeDuration> {
        ...
        public CommandResult onChange(
            ...
            try {
                DefaultTitleFadeInNode.this.set(ZonePlugin.getZonesPlugin().getConfig(),
                        new TimeDuration(DefaultTitleFadeInNode.this.getInitialValue().getTimeUnit(),
                                newValue));
    ...
    public TimeDuration getInitialValue() {
        return new TimeDuration(TimeUnits.SECONDS, 1);
    }
    ...
    public Optional<TimeDuration> get(CommentedConfigurationNode node) {
        ...
        if (length < -1 || length == 0 || unitAsString == null) {
